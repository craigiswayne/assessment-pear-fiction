...
export class Game {
    ...
    buildGame() {
        ...
        this.winText = new Text({
            text: '',
            style: { fontFamily: 'Arial', fontSize: 28, fill: 0xffd700, align: 'center', wordWrap: true, wordWrapWidth: 800 }
        });
    ...
    displayWins(winResult: WinResult) {
        ...
        onResize() {
            if (!this.gameContainer || !this.winText) return;
            ...
            this.spinButton.x = 0;
            this.spinButton.y = reelsHeight + spinButtonHeight / 2 + spinButtonGap;

            const topContentHeight = this.spinButton.y + this.spinButton.height / 2;
            const topContentWidth = Math.max(reelsWidth, this.spinButton.width);
        ...
        // Center the top part vertically in its allocated space
